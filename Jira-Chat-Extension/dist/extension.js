...
const fs = require("fs");
const BASE_PROMPT = "You are a helpful Jira assistant. You can retrieve, comment on, update, and create Jira issues. Be concise and return markdown-formatted responses.";
let currentIssueKey = null;
const output = vscode.window.createOutputChannel("Jira Extension Logs");
function activate(context) {
    const handler = async (request, chatContext, stream, token) => {
        const config = vscode.workspace.getConfiguration();
        const domain = config.get("jiraChat.domain");
        const email = config.get("jiraChat.email");
        const apiToken = config.get("jiraChat.apiToken");
...
async function handleJiraCommand(command, args, { currentIssueKey, domain, email, apiToken, context, stream, request, token, }) {
...
function generateFilesFromMarkdown(markdown, stream) {
    ...
    for (const match of codeBlocks) {
        ...
        if (!relativePath || !cleanedCode)
            continue;
        const absPath = path.join(vscode.workspace.workspaceFolders?.[0].uri.fsPath || "", relativePath);
        ...
        if (dir.includes("/aura/")) {
            if (ext === ".cmp" && !metaWritten.has(relativePath)) {
                const metaPath = path.join(vscode.workspace.workspaceFolders?.[0].uri.fsPath || "", `${dir}/${baseName}.cmp-meta.xml`);
        ...
        // Handle LWC Components
            ...
            const metaPath = path.join(vscode.workspace.workspaceFolders?.[0].uri.fsPath || "", `${dir}/${baseName}.js-meta.xml`);
...
function buildMetaXML(relativePath) {
    const ext = path.extname(relativePath);
...
async function getJiraIssue(domain, email, token, issueKey) {
    const response = await (0, node_fetch_1.default)(`${domain}/rest/api/3/issue/${issueKey}`, {
        headers: {
...
async function addJiraComment(domain, email, token, issueKey, comment) {
    const response = await (0, node_fetch_1.default)(`${domain}/rest/api/3/issue/${issueKey}/comment`, {
        method: "POST",
        headers: {
            Authorization: "Basic " + Buffer.from(`${email}:${token}`).toString("base64"),
...
async function updateJiraSummary(domain, email, token, issueKey, summary) {
    const response = await (0, node_fetch_1.default)(`${domain}/rest/api/3/issue/${issueKey}`, {
        method: "PUT",
        headers: {
            Authorization: "Basic " + Buffer.from(`${email}:${token}`).toString("base64"),
...
async function updateJiraField(domain, email, token, issueKey, field, value) {
    const fieldPayload = {};
    ...
    if (!response.ok)
        throw new Error(`Failed to update field '${field}': ${response.statusText}`);
}
async function createJiraSubtask(domain, email, token, parentKey, summary) {
    const projectKey = parentKey.split("-")[0];
    const metaResponse = await (0, node_fetch_1.default)(`${domain}/rest/api/3/issue/createmeta?projectKeys=${projectKey}&expand=projects.issuetypes.fields`, {
        headers: {
            Authorization: "Basic " + Buffer.from(`${email}:${token}`).toString("base64"),
            Accept: "application/json",
        },
    ...
    const subtaskType = meta.projects?.[0]?.issuetypes?.find((t) => t.subtask === true);
    if (!subtaskType)
        throw new Error("⚠️ This project doesn't support sub-tasks.");
    const response = await (0, node_fetch_1.default)(`${domain}/rest/api/3/issue`, {
        method: "POST",
        headers: {
            Authorization: "Basic " + Buffer.from(`${email}:${token}`).toString("base64"),
            "Content-Type": "application/json",
...
function getAssistantHistory(context) {
...
function getLastAssistantMarkdown(context) {
